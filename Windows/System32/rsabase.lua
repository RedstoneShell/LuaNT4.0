...
local function permute(block, table)
    local result = 0
    for i = 1, #table do
        local bit_pos = table[i]
        local bit = (block >> (64 - bit_pos)) & 1
        result = (result << 1) | bit
    end
    return result
end
local function permute_28(block, table)
    local result = 0
    for i = 1, #table do
        local bit_pos = table[i]
        local bit = (block >> (28 - bit_pos)) & 1
        result = (result << 1) | bit
    end
    return result
end
local function permute_48(block, table)
    local result = 0
    for i = 1, #table do
        local bit_pos = table[i]
        local bit = (block >> (56 - bit_pos)) & 1
        result = (result << 1) | bit
    end
    return result
end
...
local function pad_pkcs5(data, block_size)
    local padding = block_size - (#data % block_size)
    return data .. string.rep(string.char(padding), padding)
end

local function unpad_pkcs5(data)
    local padding = string.byte(data, #data)
    return data:sub(1, #data - padding)
end
...
crypto.rc2 = {
    expand_key = function(key, key_bits)
        ...
        if key_bits < 256 then
            ...
            local b = PI[key_bytes[key_length] & 0xFF]
            if key_length <= 128 then
                key_bytes[key_length] = b & (0xFF >> (8 - (key_bits % 8)))
            end
    ...
    encrypt_block = function(key_schedule, plaintext)
        if #plaintext ~= 8 then
            error("RC2: plaintext must be 8 bytes (64 bits)")
        end

        local bytes = string_to_bytes(plaintext)
        local words = bytes_to_words(bytes)
        local R0, R1, R2, R3 = words[1], words[2], words[3], words[4]

        local i = 1
        for r = 1, 8 do
            R0 = (R0 + key_schedule[i] + (R1 & R2) | ((~R1) & R3)) & 0xFFFF
            R0 = rotl16(R0, 1)
            R0 = R0 & 0xFFFF

            R1 = (R1 + key_schedule[i + 1] + (R2 & R3) | ((~R2) & R0)) & 0xFFFF
            R1 = rotl16(R1, 2)
            R1 = R1 & 0xFFFF

            R2 = (R2 + key_schedule[i + 2] + (R3 & R0) | ((~R3) & R1)) & 0xFFFF
            R2 = rotl16(R2, 3)
            R2 = R2 & 0xFFFF

            R3 = (R3 + key_schedule[i + 3] + (R0 & R1) | ((~R0) & R2)) & 0xFFFF
            R3 = rotl16(R3, 5)
            R3 = R3 & 0xFFFF
        ...
        local cipher_words = {R0, R1, R2, R3}
        return words_to_bytes(cipher_words)
    end,
    decrypt_block=function(key_schedule, ciphertext)
        if #ciphertext ~= 8 then
            error("RC2: ciphertext must be 8 bytes (64 bits)")
        end
        
        local bytes = string_to_bytes(ciphertext)
        local words = bytes_to_words(bytes)
        local R0, R1, R2, R3 = words[1], words[2], words[3], words[4]
        ...
        key_schedule = key_rev
        
        for r = 1, 8 do
            R3 = rotr16(R3, 5)
            R3 = (R3 - (R0 & R1) | ((~R0) & R2)) & 0xFFFF
            R3 = (R3 - key_schedule[i + 3]) & 0xFFFF
            
            R2 = rotr16(R2, 3)
            R2 = (R2 - (R3 & R0) | ((~R3) & R1)) & 0xFFFF
            R2 = (R2 - key_schedule[i + 2]) & 0xFFFF
            
            R1 = rotr16(R1, 2)
            R1 = (R1 - (R2 & R3) | ((~R2) & R0)) & 0xFFFF
            R1 = (R1 - key_schedule[i + 1]) & 0xFFFF
            
            R0 = rotr16(R0, 1)
            R0 = (R0 - (R1 & R2) | ((~R1) & R3)) & 0xFFFF
            R0 = (R0 - key_schedule[i]) & 0xFFFF
        ...
        local plain_words = {R0, R1, R2, R3}
        return words_to_bytes(plain_words)
    end,
    encrypt=function (key, data, key_bits)
        ...
        if pad_len ~= 8 then
            padded = data .. string.rep(string.char(pad_len), pad_len)
        else
            padded = data .. string.rep(string.char(8), 8)
        end
    ...
    decrypt=function (key, data, key_bits)
        ...
        local pad_len = string.byte(plaintext, #plaintext)
        if pad_len > 0 and pad_len <= 8 then
            return plaintext:sub(1, #plaintext - pad_len)
        end
        return plaintext
    end
}
...
crypto.des = {
    generate_keys=function(key)
        ...
        for i = 1, 16 do
            ...
            local combined = (C << 28) | D
            round_keys[i] = permute_48(combined, PC2)
        end
    ...
    encrypt_block=function(key, plaintext)
        ...
        for i = 1, 16 do
            ...
            for j = 1, 48 do
                local bit_pos = E[j]
                local bit = (R >> (32 - bit_pos)) & 1
                expanded = (expanded << 1) | bit
            end
            local xored = expanded ~ round_keys[i]
            local substituted = sbox_substitute(xored)
            
            local permuted = 0
            for j = 1, 32 do
                local bit_pos = P[j]
                local bit = (substituted >> (32 - bit_pos)) & 1
        ...
        local ciphertext_num = permute(combined, IP_INV)
        local ciphertext = ""
        for i = 1, 8 do
            local byte = (ciphertext_num >> (64 - i * 8)) & 0xFF
            ciphertext = ciphertext .. string.char(byte)
        end
    ...
    decrypt_block=function(key, ciphertext)
        ...
        local combined = (R << 32) | L
        local plaintext_num = permute(combined, IP_INV)
        
        local plaintext = ""
        for i = 1, 8 do
            local byte = (plaintext_num >> (64 - i * 8)) & 0xFF
            plaintext = plaintext .. string.char(byte)
        end
    ...
    decrypt=function(key, data)
        ...
        if #data % 8 ~= 0 then
            error("DES: encrypted data must be multiple of 8 bytes")
        end
    ...
    des3_encrypt=function(key, data)
        if #key ~= 16 and #key ~= 24 then
            error("3DES: key must be 16 or 24 bytes")
        end
...
crypto.md2 = function(input)
    ...
    local len = #bytes
    local pad_len = 16 - (len % 16)
    for i = 1, pad_len do
        bytes[#bytes + 1] = pad_len
    end
    local checksum = {}
    for i = 1, 16 do checksum[i] = 0 end
    local l = 0
    for i = 1, #bytes, 16 do
        for j = 0, 15 do
            local c = bytes[i + j]
            checksum[j + 1] = PI[(checksum[j + 1] ~ l ~ c) & 0xFF]
            l = checksum[j + 1]
        end
    end
    for i = 1, 16 do
...
crypto.md4 = function(input)
    ...
    local function G(x, y, z) return (x & y) | (x & z) | (y & z) end
    local function H(x, y, z) return x ~ y ~ z end
    local function rotl(x, n) return ((x << n) | (x >> (32 - n))) & 0xFFFFFFFF end
    
    for i = 1, #bytes, 64 do
        local M = {}
        for j = 0, 15 do
            M[j + 1] = (bytes[i + j * 4 + 3] << 24) | (bytes[i + j * 4 + 2] << 16) |
                       (bytes[i + j * 4 + 1] << 8) | bytes[i + j * 4]
        end
        local a, b, c, d = A, B, C, D
        local function round1(a, b, c, d, k, s) return rotl((a + F(b, c, d) + M[k]) & 0xFFFFFFFF, s) end
        local function round2(a, b, c, d, k, s) return rotl((a + G(b, c, d) + M[k] + 0x5A827999) & 0xFFFFFFFF, s) end
        local function round3(a, b, c, d, k, s) return rotl((a + H(b, c, d) + M[k] + 0x6ED9EBA1) & 0xFFFFFFFF, s) end
        a = round1(a, b, c, d, 1, 3)  b = round1(b, c, d, a, 2, 7)  c = round1(c, d, a, b, 3, 11) d = round1(d, a, b, c, 4, 19)
        a = round1(a, b, c, d, 5, 3)  b = round1(b, c, d, a, 6, 7)  c = round1(c, d, a, b, 7, 11) d = round1(d, a, b, c, 8, 19)
        a = round1(a, b, c, d, 9, 3)  b = round1(b, c, d, a, 10, 7) c = round1(c, d, a, b, 11, 11)d = round1(d, a, b, c, 12, 19)
        a = round1(a, b, c, d, 13, 3) b = round1(b, c, d, a, 14, 7) c = round1(c, d, a, b, 15, 11)d = round1(d, a, b, c, 16, 19)
        a = round2(a, b, c, d, 1, 3)  b = round2(b, c, d, a, 4, 5)  c = round2(c, d, a, b, 7, 9)  d = round2(d, a, b, c, 10, 13)
        a = round2(a, b, c, d, 13, 3) b = round2(b, c, d, a, 16, 5) c = round2(c, d, a, b, 3, 9)  d = round2(d, a, b, c, 6, 13)
        a = round2(a, b, c, d, 9, 3)  b = round2(b, c, d, a, 12, 5) c = round2(c, d, a, b, 15, 9) d = round2(d, a, b, c, 2, 13)
        a = round2(a, b, c, d, 5, 3)  b = round2(b, c, d, a, 8, 5)  c = round2(c, d, a, b, 11, 9) d = round2(d, a, b, c, 14, 13)
        a = round3(a, b, c, d, 1, 3)  b = round3(b, c, d, a, 6, 9)  c = round3(c, d, a, b, 11, 11)d = round3(d, a, b, c, 16, 15)
        a = round3(a, b, c, d, 5, 3)  b = round3(b, c, d, a, 10, 9) c = round3(c, d, a, b, 15, 11)d = round3(d, a, b, c, 4, 15)
        a = round3(a, b, c, d, 9, 3)  b = round3(b, c, d, a, 14, 9) c = round3(c, d, a, b, 3, 11) d = round3(d, a, b, c, 8, 15)
        a = round3(a, b, c, d, 13, 3) b = round3(b, c, d, a, 2, 9)  c = round3(c, d, a, b, 7, 11) d = round3(d, a, b, c, 12, 15)
        A = (A + a) & 0xFFFFFFFF
        B = (B + b) & 0xFFFFFFFF
        C = (C + c) & 0xFFFFFFFF
        D = (D + d) & 0xFFFFFFFF
...
crypto.md5 = function(input)
    ...
    local function T(i) return math.floor(2^32 * math.abs(math.sin(i))) & 0xFFFFFFFF end
    for i = 1, #bytes, 64 do
        local M = {}
        for j = 0, 15 do
            M[j + 1] = (bytes[i + j * 4 + 3] << 24) | (bytes[i + j * 4 + 2] << 16) |
        ...
        for j = 1, 64 do
            ...
            local s = S[(j - 1) % 4 + 1 + math.floor((j - 1) / 16) * 4]
            local t = T(j)
            a = (a + f + M[g + 1] + t) & 0xFFFFFFFF
            a = (b + rotl(a, s)) & 0xFFFFFFFF
            a, b, c, d = d, a, b, c
        end
        A = (A + a) & 0xFFFFFFFF
        B = (B + b) & 0xFFFFFFFF
